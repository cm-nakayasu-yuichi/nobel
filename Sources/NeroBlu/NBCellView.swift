...
public extension UICollectionView {
    ...
    public func dequeueReusableCell<T: NBCellRegisterable>(_ registerableType: T.Type, forIndexPath indexPath: IndexPath? = nil) -> T where T: UICollectionViewCell {
        let id = registerableType.cellIdentifier
        let reusableCell = self.dequeueReusableCell(withReuseIdentifier: id, for: indexPath ?? IndexPath(0, section: 0))
...
public extension UICollectionViewLayout {
    ...
    public class func tiledLayout(numberOfColumns column: Int = 4, interval: CGFloat = 1.0, margin: CGFloat = 1.0, totalWidth: CGFloat? = nil, direction: UICollectionView.ScrollDirection = .vertical) -> UICollectionViewLayout {
        ...
        ret.itemSize = CGSize(per, per)
        
        ret.minimumInteritemSpacing = interval
        ret.minimumLineSpacing = interval
        ret.sectionInset = UIEdgeInsets(top: margin, left: margin, bottom: margin, right: margin)
