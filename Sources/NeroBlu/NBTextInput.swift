...
open class NBKeyboardEventManager: NSObject {
    ...
    fileprivate func observeKeyboardEvents(_ start: Bool) {
        ...
        App.Notify.observe(self, start: start, notificationsAndSelectors:[
            UIResponder.keyboardWillShowNotification.rawValue        : selector,
            UIResponder.keyboardWillChangeFrameNotification.rawValue : selector,
            UIResponder.keyboardWillHideNotification.rawValue        : selector,
            ]
        )
    }
    ...
    @objc fileprivate func willChangeKeyboardFrame(_ notify: Notification) {
        guard
            ...
            let beginFrame = (userInfo[UIResponder.keyboardFrameBeginUserInfoKey] as AnyObject).cgRectValue,
